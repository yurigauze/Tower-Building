...
Sprites::Sprites(const std::string &id, const std::string &filePath,
                 ...
                 int totalFrames, float frameTime, ILogger* logger) 
      ...
      renderer(renderer), logger(logger)
{
    ...
    else
    {
        ...
        if (logger)
        {
            logger->Log("Erro ao carregar textura: " + filePath);
        }
    }
}
                     ...
                     int height)
{
    if (texture)
    {
        ...
        if (SDL_RenderCopy(renderer, texture, &srcRect, &destRect) != 0)
        {
            if (logger)
            {
                logger->Log("SDL_RenderCopy failed: " + std::string(SDL_GetError()));
            }
        }
    }
    else
                              ...
                              int height)
{
    if (texture)
    {
        SDL_Rect fullRect = {0, 0, srcRect.w, srcRect.h};
        destRect.x = x;
        destRect.y = y;
        destRect.w = width;
        destRect.h = height;
                                 ...
                                 int width, int height, double angle)
{
    ...
    else
    {
        if (logger)
        {
            logger->Log("Texture is invalid or not loaded.");
        }
    }
}
